...
public class UnityIPhoneXSupport {
    ...
    private const float headerSize = 30.0f;
    private const float footerSize = 20.0f;
    ...
    public static void OnPostProcessBuild(BuildTarget buildTarget, string path) {
		File.Copy ("Assets/Editor/iPhoneXSupport/" + headerImage, Path.Combine (path, "Data/" + headerImage), true);
		File.Copy ("Assets/Editor/iPhoneXSupport/" + footerImage, Path.Combine (path, "Data/" + footerImage), true);
        ...
        if (!string.IsNullOrEmpty(headerImage)) {
            string headerImageText = "        NSString *headerImagePath = [[NSBundle mainBundle] pathForResource:@\"Data/{0}\" ofType:nil];\n" +
        ...
        if (!string.IsNullOrEmpty(footerImage)) {
            string footerImageText = "        NSString *footerImagePath = [[NSBundle mainBundle] pathForResource:@\"Data/{0}\" ofType:nil];\n" +
