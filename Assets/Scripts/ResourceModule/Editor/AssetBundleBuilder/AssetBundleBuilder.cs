...
namespace ResourceModule
{
	public class AssetBundleBuilder : EditorWindow
	{
			...
			else
			{
				...
				"PreVerticalScrollbar");
				{
					...
					EditorGUILayout.BeginVertical("HelpBox");
					{
						...
						{
							int option = EditorUtility.DisplayDialogComplex("确认", "是否清空所有资源BundleName?", "清空未使用的", "Cancel", "全部清空");
							if (option != 1)
							{
								EditorApplication.delayCall += () =>
								{
									CleanUpBundleName(option == 2);
								};
							}
						}
						EditorGUILayout.Space();
					...
					EditorGUILayout.BeginVertical("HelpBox");
					{
						...
						if (GUILayout.Button("生成到指定版本补丁包", "LargeButton", GUILayout.Height(50f)))
						{
							EditorApplication.delayCall += GenPatch;
						}
						EditorGUILayout.Space();
						if (GUILayout.Button("递进生成所有版本补丁包", "LargeButton", GUILayout.Height(50f)))
						{
		...
		private void DrawBundleNamePanel()
		{
			...
			{
				int hitCount = 0;
				EditorGUILayout.BeginVertical("HelpBox", GUILayout.Height(150f));
				{
		...
		private static void SetupAssetBundleName(string assetPath)
		{
			...
			if (assetPath.StartsWith(ResourceModuleConfig.BundleResoucesDir))
			{
				var importer = AssetImporter.GetAtPath(assetPath);
				if (importer != null)
				{
					string bundleName = assetPath.Replace(ResourceModuleConfig.BundleResoucesDir + "/", "") + ResourceModuleConfig.AssetBundleExt;
		...
		public static void OneKeyBuildGameRes(bool updateVersion = false)
		{
			SlientMode = true;
			if (updateVersion)
			{
		...
		public static bool GenPatchPackages(string oldResDir, string newResDir)
		{
			...
			if (patchedNames.Count > 0)
			{
				...
				foreach (var patchedName in patchedNames)
				{
					...
					fileHash += MD5Hashing.HashFile(patchFile);
				}

				string zipFile = patchDir + ".zip";
				ZipTool.CompressFolder(zipFile, patchDir);
				string md5 = MD5Hashing.HashString(fileHash);
				long fileSize = new FileInfo(zipFile).Length;
				string finalZipFile = string.Format("{0}-{1}.zip", patchDir, md5);
		...
		{
			get { return ResourceModuleConfig.BundleResoucesDir + "/" + ResourceModuleConfig.ResVerFileName; }
		}
